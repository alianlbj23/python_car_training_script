...
class Env(Environment):
    ...
    def calculate_reward(self, state: Entity.State, new_state: Entity.State):
        ...
        self.trailOrientation = Utility.rad2deg(Utility.radFromUp([new_state.path_closest_pos.x, \
                                                                   new_state.path_closest_pos.y],
                                                                  [new_state.path_second_pos.x, \
...
class Agt(Agent):
    ...
    def processFeature(self, state: Entity.State, prev_pos, trail_original_pos):
        ...
        feature.append(
            state.car_pos.y - state.path_closest_pos.y)  # old: state.path_closest_pos.y, -state.path_closest_pos.x
        # print("vector")
...
def main(mode):
    ...
    env = Env(max_times_in_episode=50, max_times_in_game=250, end_distance=(0.2, 6), stop_target=False,
              target_fixed_sec=12)

    chpt_dir_load = os.path.join(os.path.dirname(__file__), './Model',
                                 '0507_curve_rear_engine_car_new_protocal_local_angularV')
